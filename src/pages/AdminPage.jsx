    ...
    <div className="space-y-4">
      ...
      {(pendingIban + pendingCards + pendingAccounts + pendingKyc + pendingActivations) > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center gap-2">
          ...
          <button type="button" onClick={() => setTab('requests')} className="text-[11px] font-semibold text-amber-900 underline whitespace-nowrap">Voir les demandes</button>
        </div>
      )}
    </div>
  );
...
function TabRequests({ users, requests, accounts, cards, cardRequests, kycSubmissions, load }) {
  ...
  const pendingIban = requests.filter(r => (r.type === 'iban_request' || r.step === 'iban_request') && r.status === 'pending');
  const pendingCards = cardRequests.filter(c => c.status === 'pending');
  const pendingActivations = requests.filter(r => r.step === 'transfer_proof' && r.status === 'pending');
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-[18px] font-semibold">Demandes en attente</h1>
      
      {/* Navigation par sections */}
      <div className="flex gap-2 border-b border-slate-200">
        {[
          { id: 'accounts', label: `Comptes (${pendingAccounts.length})`, count: pendingAccounts.length },
          { id: 'kyc', label: `KYC (${pendingKyc.length})`, count: pendingKyc.length },
          { id: 'iban', label: `IBAN (${pendingIban.length})`, count: pendingIban.length },
          { id: 'cards', label: `Cartes (${pendingCards.length})`, count: pendingCards.length },
          { id: 'activations', label: `Activations (${pendingActivations.length})`, count: pendingActivations.length }
        ].map(section => (
          <button
            key={section.id}
      ...
      {activeSection === 'accounts' && (
        <div className="space-y-3">
          {pendingAccounts.length === 0 ? (
            <p className="text-slate-500 text-center py-8">Aucun compte en attente</p>
          ) : (
            pendingAccounts.map(user => (
              <div key={user.id} className="bg-white border border-slate-100 rounded-xl p-4">
      ...
      {/* Section KYC en attente */}
      {activeSection === 'kyc' && (
        <div className="space-y-3">
          {pendingKyc.length === 0 ? (
            <p className="text-slate-500 text-center py-8">Aucune demande KYC en attente</p>
          ) : (
            pendingKyc.map(kyc => (
              <div key={kyc.id} className="bg-white border border-slate-100 rounded-xl p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">{kyc.name || kyc.email}</p>
      ...
      {/* Section IBAN en attente */}
      {activeSection === 'iban' && (
        <div className="space-y-3">
          {pendingIban.length === 0 ? (
            <p className="text-slate-500 text-center py-8">Aucune demande IBAN en attente</p>
          ) : (
            pendingIban.map(request => (
              <div key={request.id} className="bg-white border border-slate-100 rounded-xl p-4">
                <div className="mb-4">
                  <p className="font-medium text-[13px]">{request.name || request.email}</p>
                  <p className="text-[11px] text-slate-500">{request.email}</p>
                </div>
                ...
                <div className="space-y-3 border-t pt-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        value={manualIbanForm[request.user_id]?.iban || ''}
                        onChange={(e) => setManualIbanForm(prev => ({ 
                          ...prev, 
                          [request.user_id]: { ...prev[request.user_id], iban: e.target.value } 
                        }))}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={manualIbanForm[request.user_id]?.bic || ''}
                        onChange={(e) => setManualIbanForm(prev => ({ 
                          ...prev, 
                          [request.user_id]: { ...prev[request.user_id], bic: e.target.value } 
                        }))}
                        className="w-full px-2 py-1.5 border border-slate-200 rounded text-[11px] focus:outline-none focus:border-teal-400"
                        style={{ fontFamily: 'monospace' }}
                      />
                  ...
                  <button
                    onClick={() => handleApproveIban(request.user_id)}
          ...
          {pendingCards.length === 0 ? (
            ...
            pendingCards.map(card => (
              <div key={card.id} className="bg-white border border-slate-100 rounded-xl p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">Carte pour {card.email}</p>
                    <p className="text-sm text-slate-500">Type: {card.type || 'Standard'}</p>
                  </div>
                  <button
                    className="px-4 py-2 bg-teal-600 text-white rounded-lg text-sm hover:bg-teal-700"
                  >
      ...
      {/* Section Activations en attente */}
      {activeSection === 'activations' && (
        <div className="space-y-3">
          {pendingActivations.length === 0 ? (
            <p className="text-slate-500 text-center py-8">Aucune activation en attente</p>
          ) : (
            pendingActivations.map(activation => (
              <div key={activation.id} className="bg-white border border-slate-100 rounded-xl p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">{activation.name || activation.email}</p>
                    <p className="text-sm text-slate-500">Montant: {fmt(activation.amount)}</p>
                  </div>
                  <button
                    className="px-4 py-2 bg-teal-600 text-white rounded-lg text-sm hover:bg-teal-700"
                  >
